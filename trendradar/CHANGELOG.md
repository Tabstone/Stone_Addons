# Changelog

## 6.10.0-1

- Sync upstream image [wantcat/trendradar:6.10.0](https://hub.docker.com/r/wantcat/trendradar).
- Upstream project: [sansan0/TrendRadar](https://github.com/sansan0/TrendRadar).
- Upstream tag: [v6.10.0](https://github.com/sansan0/TrendRadar/tree/v6.10.0).


## 6.9.1-1

- Sync upstream image to 6.9.1.


## 6.9.0-1

- Sync upstream image to 6.9.0.


## 6.8.1-1

- Sync upstream image to 6.8.1.


## 6.8.0-1

- Sync upstream image to 6.8.0.


## 6.7.1-1

- Sync upstream image to 6.7.1.


## 6.7.0-1

- Sync upstream image to 6.7.0.


## 6.6.2-1

- Sync upstream image to 6.6.2.


## 6.6.1-1

- Sync upstream image to 6.6.1.


## 6.6.0-1

- Sync upstream image to 6.6.0.


## 6.5.5-1

- Sync upstream image to 6.5.5.


## 6.5.3-1

- Sync upstream image to 6.5.3.


## 6.5.2-1

- Sync upstream image to 6.5.2.


## 6.5.1-1

- Sync upstream image to 6.5.1.


## 6.5.0-1

- Sync upstream image to 6.5.0.


## 6.0.0-3

- Sync upstream image to 6.0.0.


## 6.0.0-2

- Bump add-on revision to validate Home Assistant update detection.

## 6.0.0-1

- Initial Home Assistant add-on wrapper release for TrendRadar.
- Added persistent config and sibling output directory mapping.
- Added Ingress and direct port access.
